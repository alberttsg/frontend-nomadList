    ...
    toggleVisited(countryId).then(res => {
      setCountries(previousCountries =>
        previousCountries.map(country => {
          if (country._id === res._id) { return res }
          else { return country }
        }))
    })
    getUserInfo()
                ...
                <Card.Meta key={country._id}
                  title={
                    <Space direction='horizontal' align='baseline'>
                      <span style={{ fontSize: '18px' }}>{country.country}</span>
                      {user?.visited?.some((user) => user._id === country._id) ? (
                        <CheckCircleTwoTone twoToneColor="#52c41a" style={{ fontSize: '22px' }} onClick={() => toggleVisit(country._id)} />
                      ) : (
                        <PlusCircleTwoTone twoToneColor="lightgray" style={{ fontSize: '22px' }} onClick={() => toggleVisit(country._id)} />
                      )}
                    </Space>
                  }
                  avatar={
                    <Avatar.Group
                      maxCount={1}
                      onMaxPopoverVisibleChange={() => setModalVisible(true)}
